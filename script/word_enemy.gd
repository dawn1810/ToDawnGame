...
func spawn_enemies(index, pos):
	...
	# set up key location
	set_up_new_key(index, instance.rand_color)
...
func set_up_new_key(index, color):
	keys.add_child(Label.new())
	# random color for key lable of curr sprite (base on sprite color)
	match color:
		'idle':
			keys.get_child(index).modulate = '5c2a00'
		'idle_2':
			keys.get_child(index).modulate = 'ffeddc'
